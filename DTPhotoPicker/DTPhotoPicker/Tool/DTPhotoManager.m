...
@property (nonatomic ,strong) NSMutableArray<DTAblumInfo *> * ablumsList;

@end

@implementation DTPhotoManager

static DTPhotoManager *manager = nil;

+ (instancetype)manager
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[DTPhotoManager alloc]init];
...
- (NSArray<DTAblumInfo *> *)getAllAblums
{
    ...
    //列出并加入所有智能相册
    PHFetchResult *smartAblums = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum
...
- (void)fetchCollection:(PHFetchResult *)obj
{
    ...
    [obj enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
    {
        if ([obj isKindOfClass:[PHAssetCollection class]])
        {
            ...
            {
                //创建此相册的信息集
                DTAblumInfo *info = [DTAblumInfo infoFromResult:result collection:obj];
                //加入到数组中
...
- (void)fetchImagesWithAssetsArray:(NSMutableArray<PHAsset *> *)assetsArray
                        isOriginal:(BOOL)isOriginal
                     completeBlock:(void(^)(NSArray * images))completeBlock
{
    
    NSMutableArray * images = [NSMutableArray array];
    NSLog(@"*********************图片个数:%ld",assetsArray.count);
    ...
    for (int i = 0; i < assetsArray.count; i ++)
    {
        ...
        else
        {
            ...
            if (scale > 1.0)
            {
                if (asset.pixelWidth < SCREEN_WIDTH)
                {
                    //最长边小于屏幕宽度时，采用原图
                    size = CGSizeMake((CGFloat)asset.pixelWidth, (CGFloat)asset.pixelHeight);
                }
                ...
                {
                    //压缩
                    size = CGSizeMake(SCREEN_WIDTH, SCREEN_WIDTH / scale);
                }
            ...
            else
            {
                ...
                if (asset.pixelHeight < SCREEN_HEIGHT)
                {
                    //最长边小于屏幕高度时，采用原图
                    size = CGSizeMake((CGFloat)asset.pixelWidth, (CGFloat)asset.pixelHeight);
                }
                ...
                {
                    //压缩
                    size = CGSizeMake(SCREEN_HEIGHT * scale, SCREEN_HEIGHT);
                }
